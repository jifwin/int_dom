...
void pit_init(void){
	...
	NVIC_ClearPendingIRQ(PIT_Number);				/* Clear NVIC any pending PIT interrupts */
	NVIC_EnableIRQ(PIT_Number);							/* Enable NVIC interrupts source for PIT module */
	NVIC_SetPriority (PIT_Number, 3);			/* Set PIT interrupt priority level  */ 
