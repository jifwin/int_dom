...
void ADC0_IRQHandler(void)
{
		...
		TPM0->CONTROLS[2].CnV = TPM_CnV_VAL(value); 
		TPM0->CONTROLS[5].CnV = TPM_CnV_VAL(value); 
...
void ADC_init(void)
{
	...
	if(!ADC_calibrate()){ //success
		...
		//ADC0->SC1[0] |= ADC_SC1_DIFF_MASK; 	//DIFF=1
		
		ADC0->SC1[0] = ADC_SC1_ADCH(26)| ADC_SC1_AIEN_MASK;	 //przerwanie aktywne DADP3 is selected as input;
